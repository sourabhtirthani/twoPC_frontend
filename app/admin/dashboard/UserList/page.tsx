  ...
  return (
    <div className="relative ml-6">
      ...
      <div className="flex items-center gap-2 py-2 group">
        <div className={`w-8 h-8 rounded-full flex items-center justify-center border transition-all ${isRoot ? "bg-blue-600 border-blue-400 shadow-lg shadow-blue-100" : "bg-slate-100 border-slate-200"
          }`}>
...
export default function UserList() {
  ...
  return (
    <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden">
      ...
      {/* --- TOP BAR --- */}
      <div className="p-6 flex flex-wrap gap-y-4 justify-between items-center border-b bg-slate-50/50">
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredUsers.map((user, idx) => (
              <tr key={user._id || idx} className="hover:bg-blue-50/30 transition-colors">
                ...
                <td className="px-8 py-5">
                  <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-tighter ${user.role === 'ADMIN' ? 'bg-purple-100 text-purple-600' : 'bg-slate-100 text-slate-600'
                    }`}>
                    {user.role || "User"}
                  </span>
                </td>
